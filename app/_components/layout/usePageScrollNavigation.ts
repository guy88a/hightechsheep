...
export type NavigationZone = "top" | "bottom" | null;
export type NavigationIntent = "nav-click" | null;

type UsePageScrollNavigationOptions = Readonly<{
...
export function usePageScrollNavigation({
  ...
  const getNavigationZone = useCallback(
    (main: HTMLElement): NavigationZone => {
    ...
    [getNavigationZoneSize],
  );

  const getRequestedDirection = useCallback(
    (navigationZone: NavigationZone): Exclude<ScrollDirection, null> | null => {
      if (wheelDirectionRef.current === "next" && navigationZone === "bottom") {
        return "next";
      }

      if (
        wheelDirectionRef.current === "previous" &&
        navigationZone === "top"
      ) {
        return "previous";
      }
  ...
  const clearNavigationCooldown = useCallback(() => {
  ...
  const isWebsiteBoundary = useCallback(
  ...
  const resetNavigationZone = useCallback(
    (navigationZone: NavigationZone) => {
    ...
    [clearNavigationCooldown, isWebsiteBoundary],
  );

  const syncNavigationZone = useCallback(
    (main: HTMLElement) => {
  ...
  const handleScroll = useCallback(
    ...
    [syncNavigationZone, updateScrollMetrics],
  );
